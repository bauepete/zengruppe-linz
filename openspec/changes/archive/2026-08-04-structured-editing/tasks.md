...
- [x] C.2 Decide and document whether `pageType` should remain supported; if yes, add it explicitly to the spec, otherwise remove it from implementation.
- [x] C.3 Align OpenSpec artifacts on quote support by ensuring proposal/tasks/spec text consistently document `type: quote` and optional `author`.
- [x] C.4 Specify and verify inline link normalization behavior (supported forms, unsupported-form handling, and base-path-safe resolution) in spec scenarios.
